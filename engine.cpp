    ...
    switch (order.type)
    {
        ...
        { // BUY
            ...
            else
            {
                ...
                //     printf("%s %u @ $%u.%02u | ID %u | %s | %s\n", e.type == 1 ? "Fill" : "Cancel", e.quantity, e.price/100, e.price%100, e.orderID, e.side == 0 ? "Buy" : "Sell", e.fullyFilled ? "Full" : "Partial");
                // }
        ...
        case 2: // CANCEL
        {
            ...
            // temp print
            Event cancelEvent = {0, 0, order.orderID, 0, 1}; // cancel event
