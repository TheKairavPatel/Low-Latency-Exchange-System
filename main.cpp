...
static constexpr int BOOK_DEPTH = 500; // orders to keep in book at all times

uint16_t freeIDs[65536];
uint16_t topID = 0;
inline void initIDs() { for (int i = 0; i < 65536; i++) freeIDs[i] = i; topID = 0; }
...
// ── PREGEN ──
enum OpType : uint8_t { PASSIVE_BUY, PASSIVE_SELL, AGGRESSIVE_BUY, AGGRESSIVE_SELL, CANCEL_BID, CANCEL_ASK };
...
int main()
{
    ...
    // price distributions — clustered around mid
    std::normal_distribution<double> bidOffDist(15.0, 5.0);  // passive bids 10-20 ticks below mid
    std::normal_distribution<double> askOffDist(15.0, 5.0);  // passive asks 10-20 ticks above mid
    std::uniform_int_distribution<uint32_t> aggOffDist(1, 5); // aggressive crosses by 1-5 ticks
    std::uniform_int_distribution<uint32_t> qtyDist(1, 300);

    // op mix — realistic market microstructure
    // 35% passive buy, 35% passive sell, 10% aggressive buy, 10% aggressive sell, 5% cancel bid, 5% cancel ask
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        switch (type)
        {
            ...
            case PASSIVE_SELL:
                ...
                break;
            case AGGRESSIVE_BUY:
                price = BASE + 200 + askOff + aggOffDist(rng); // crosses ask
                break;
            case AGGRESSIVE_SELL:
                price = BASE + 200 - bidOff - aggOffDist(rng); // crosses bid
                break;
            case CANCEL_BID:
            case CANCEL_ASK:
                price = 0; qty = 0; // resolved at runtime from live book
                break;
        }
    ...
    // ── WARMUP — build initial book depth ──
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        switch (op.type)
        {
            case PASSIVE_BUY:
            {
                ...
                // only track as live if it didn't match (check best ask)
    ...
    // filter outliers before sorting
    samples.erase(std::remove_if(samples.begin(), samples.end(), [](uint64_t x) {
    return x > 100000;
}), samples.end());
